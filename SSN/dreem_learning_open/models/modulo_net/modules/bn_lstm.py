...
"""Very Strongly inspired by https://github.com/pytorch/benchmark/blob/master/rnns/benchmarks/models
/bnlstm.py"""
import torch
...
class SeparatedBatchNorm1d(nn.Module):
    ...
    def forward(self, input_, time):
        ...
        if input_.shape[0] > 1:
            ...
            return functional.batch_norm(
                input=input_, running_mean=running_mean, running_var=running_var,
                weight=self.weight, bias=self.bias, training=False,
                momentum=self.momentum, eps=self.eps)
...
class LSTM(nn.Module):
    ...
    def __init__(self, cell_class, input_size, hidden_size, num_layers=1,
                 use_bias=True, batch_first=False, dropout=0, backward=False, **kwargs):
    ...
    def forward(self, input_, length=None, hx=None):
        ...
        if self.backward:
            input_ = torch.stack(reverse(input_.unbind(0)))
        ...
        output = layer_output
        output = torch.stack(reverse(output.unbind(0)))
