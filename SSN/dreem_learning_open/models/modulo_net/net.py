...
class ModuloNet(nn.Module):
    ...
    def predict_on_record(self, record: str, dataset: DreemDataset, stride: int = 1,
        ...
        if return_prob:
            ...
            return np.argmax(predicted_output, -1), np.exp(predicted_output) / np.sum(np.exp(predicted_output), -1,
                                                     keepdims=True)
    ...
    def predict_on_dataset(self, dataset: DreemDataset, stride: int = 1, return_prob: bool = False,
        ...
        for record in enumerator:
            result[f"{record}\labels"], result[f"{record}\prob"] = self.predict_on_record(record, dataset, stride, return_prob, mode)
        return result
