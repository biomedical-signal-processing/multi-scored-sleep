...
class SimpleSpectrogramEncoder(nn.Module):
    ...
    def __init__(self, input_dim=129, n_channels=1, hidden_layers=64, bidir=True, num_layers=1,
        ...
        self.virtual_channels = nn.Linear(n_channels, n_channels)

        self.num_layers = num_layers
        self.gru = nn.GRU(input_size=input_dim * n_channels, hidden_size=self.hidden_size,
    ...
    def forward(self, x):
        ...
        x = self.virtual_channels(x).transpose(1, 3)
        # x = self.dropout(x)
        # Dropout2D
        x = self.dropout(x)
...
class SimpleSleepNetEpochEncoderWithoutFrequencyReduction(EpochEncoder):
    ...
    def __init__(self, group, net_parameters):
        ...
        """
        super(SimpleSleepNetEpochEncoderWithoutFrequencyReduction, self).__init__(group,
                                                                                  net_parameters)
