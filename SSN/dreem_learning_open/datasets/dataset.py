...
class DreemDataset(Dataset):
    ...
    def __getitem__(self, idx, mode='train'):
        ...
        # Retrieve Soft-Consensus
        distribution = np.load(f"{record}/soft_consensus.npz")["soft_consensus"]
        sample['distribution'] = torch.FloatTensor(
            distribution[start_idx:end_idx, :]
        )
