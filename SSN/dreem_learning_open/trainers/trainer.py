...
class Trainer:
    ...
    def validate(self, validation_dataset, return_metrics_per_records=False, verbose=False):
        ...
        metrics_per_records = {}
        hypnograms = {}
        # Change to return label and probabilities
        predictions = self.net.predict_on_dataset(validation_dataset, return_prob=True,
                                                  verbose=verbose)
        record_weights = []
    ...
    def train_on_batch(self, data, mask=-1):
        ...
        # Retrieve inputs
        if self.ls["type"] == "empirical":
            args, hypnogram = self.net.get_args(data)
            device = self.net.device
            hypnogram = hypnogram.to(device)
            h = np.array(hypnogram.cpu())
            mask = [hypnogram != mask]
            hypnogram = hypnogram[mask]
            distribution = np.vstack(np.array(data['distribution']))
            idx = np.where(h != -1)
            ...
            hypnogram = hypnogram[mask]
        # Apply LSU
        if self.ls["type"] == "uniform":
            hypnogram_loss = self.label_smoothing_uniform(hypnogram)
            hypnogram_loss = hypnogram_loss.to(device)
        elif self.ls["type"] == "empirical":
            hypnogram_loss = self.label_smoothing_empirical(hypnogram, distribution)
            hypnogram_loss = hypnogram_loss.to(device)
        ...
        if self.ls["type"] == "empirical" or self.ls["type"] == "uniform":
            loss_train = self.loss_function(output, hypnogram_loss)
        else:
    ...
    def label_smoothing_uniform(self, target, K=5):
        return self.alpha * (1 / K) + (1 - self.alpha) * one_hot(target, num_classes=K)

    def label_smoothing_empirical(self, target, distribution):
        return FloatTensor(self.alpha * np.vstack(distribution) + (1 - self.alpha) * np.vstack(one_hot(target, num_classes=5).cpu()))
